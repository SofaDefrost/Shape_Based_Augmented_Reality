...
# Calibration matrix
calibration_matrix = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]], dtype=np.float32)

camera_intrinsics = o3d.camera.PinholeCameraIntrinsic(
    width=848, height=480, fx=fx, fy=fy, cx=cx, cy=cy
)
...
try:
    while True:
        ...
        depth_image_bg_removed = np.where(
            (depth_image > clipping_distance) | (depth_image <= 0), 0, depth_image
        )
        # Render images:
