...
def project_3D_model_on_pc_using_closest_points_identification(points_model_3D: np.ndarray, colors_model_3D: np.ndarray, points_pc: np.ndarray, colors_pc: np.ndarray, projection_matrix: np.ndarray, shape_image: Tuple[int, int]) -> np.ndarray:
    """
    ...
    _, indices_of_closest_points = pc.find_nearest_neighbors_between_pc(
        points_pc, model_3D_points_after_projection, 1)
    ...
    for indice in indices_of_closest_points:
        ...
        for offset in [-1, 0, 1, -shape_image[0], -shape_image[0] - 1, -shape_image[0] + 1, shape_image[0], shape_image[0] - 1, shape_image[0] + 1]:
...
def project_3D_model_on_pc_using_closest_points_and_indices(points_model_3D: np.ndarray, colors_model_3D: np.ndarray, points_pc: np.ndarray, colors_pc: np.ndarray, table_of_indice_pc: np.ndarray, projection_matrix: np.ndarray, shape_image: Tuple[int, int]) -> np.ndarray:
    """
...
def project_3D_model_on_pc(frame: np.ndarray, points: np.ndarray, colors: np.ndarray, projection_matrix: np.ndarray, shape_image: Tuple[int, int]) -> np.ndarray:
    """
    ...
    for i, p in enumerate(projected_points.astype(int)):
        if 0 <= p[0] < frame.shape[1] and 0 <= p[1] < frame.shape[0]:
            color = colors[i]
            color = (int(color[2]), int(color[1]),
                     int(color[0]))  # Convert RGB
