...
from Python_3D_Toolbox_for_Realsense import processing_multiple_ply as mply
...
# Parce que l'objet n'est pas dans le bon sens 

angle = np.radians(180)
Mat =tf.rotation_matrix_y(angle)    
points_model_3D= np.array([(float(x), float(y), float(z)) for (
x, y, z) in [Mat @ point for point in points_model_3D ]])
...
size_acqui = (1280, 720)
points,colors=mply.get_point_cloud("Python_3D_Toolbox_for_Realsense/test2.mply",1)
...
fps = 25
video_writer = cv2.VideoWriter('example/output/test_video_post_prod.mp4', cv2.VideoWriter_fourcc(*'XVID'), fps, (width, height))

# # Needed because the code that get the calibration matrix reset the pipeline
# pipeline = aq.init_realsense(size_acqui[0], size_acqui[1])

file = open("Python_3D_Toolbox_for_Realsense/test2.mply", "r")
